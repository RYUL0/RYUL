...
ikson_count1 = 0 # 매매 사이클 카운트
ikson_count2 = 0

trading_count1 = 0 # 매매 카운트
trading_count2 = 0
...
ikson_start = 9             # 시작 익절 카운트
ikson_stop = 8              # 정지 익절 카운트

token = '6027720676:AAH7TLZOeikzXi3uuo7tMDTfBifhud6wMdk'
chat_id = '6012236354'

token2 = '6199814629:AAEe6VU6PJdbPaa8JrQQhHm-DFRfGkhyqHI'
chat_id2 = '6012236354'
...
async def main_정산_데이터갱신(): #실행시킬 함수명 임의지정
    while True:
        try:
            ...
            await bot.send_message(chat_id2, f"누적 {count1_0 + count1_1}/{count2_1}\n갱신 {ikson_list.count(1)}/{ikson_list.count(0)}")
            break
        except:
            await asyncio.sleep(timesleep)
            continue
...
async def main_정산_매매(): #실행시킬 함수명 임의지정
    while True:
        try:
            bot = telegram.Bot(token)
            await bot.send_message(chat_id, f"누적 {trading_count1}/{trading_count2}\n사이클 {ikson_count1}/{ikson_count2}")
            break
        except:
            await asyncio.sleep(timesleep)
            continue
